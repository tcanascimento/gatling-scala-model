...
class TestHeaviSideUsersDuringTimePost extends Simulation with SpecTrait{
  ...
  setUp(
    ...
    scene.inject(heavisideUsers(simulSpecs.initialUsers)
      .during(simulSpecs.duration)).protocols(http.baseUrl(specs.baseUrl))
